...
<div id="wrap">
			...
			<p align="right"><a href="/admin/recruitWrite"><img src="/admin/img/btn_apply.gif"></a></p>
			...
			<table class="aborad01">
				...
				<c:forEach var="recruitRecord" items="${recruitList}">
					<tr>
						<td><input type="checkbox" name="boardNum" value="recruit&${recruitRecord.num}"/></td>
						<td>${recruitRecord.num}</td>
						<td>${recruitRecord.division}</td>
						<td>${recruitRecord.career}</td>
						<td class="tleft"><a href="/admin/recruitView?num=${recruitRecord.num}&pageNum=${page}">${recruitRecord.title}</a></td>
						<td>${recruitRecord.startDate}~${recruitRecord.endDate}</td>
						<td>${recruitRecord.date}</td>
					</tr>
				</c:forEach>
			</table>
